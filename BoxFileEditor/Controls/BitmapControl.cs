...
namespace BoxFileEditor
{
    ...
    /// </summary>
    public class BitmapControl : UIElement
    {
        public static readonly DependencyProperty SourceProperty = DependencyProperty.Register(
            "Source",
            typeof(BitmapSource),
            typeof(BitmapControl),
        ...
        {
            var bmpCtl = (BitmapControl)d;

            var oldValue = (BitmapSource)e.OldValue;
            var newValue = (BitmapSource)e.NewValue;

            if (((oldValue != null) && (bmpCtl._sourceDownloaded != null)) &&
                (!oldValue.IsFrozen && (oldValue is BitmapSource)))
            {
                (oldValue).DownloadCompleted -= bmpCtl._sourceDownloaded;
                (oldValue).DownloadFailed -= bmpCtl._sourceFailed;
                // ((BitmapSource)newValue).DecodeFailed -= bitmap._sourceFailed; // 3.5
            }
            if (((newValue != null) && (newValue is BitmapSource)) && !newValue.IsFrozen)
            {
                (newValue).DownloadCompleted += bmpCtl._sourceDownloaded;
                (newValue).DownloadFailed += bmpCtl._sourceFailed;
                // ((BitmapSource)newValue).DecodeFailed += bitmap._sourceFailed; // 3.5
            }
        }
